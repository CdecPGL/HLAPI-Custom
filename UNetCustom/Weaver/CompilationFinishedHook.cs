    ...
    public class CompilationFinishedHook {
        static CompilationFinishedHook() {
            ...
            CompilationPipeline.assemblyCompilationFinished += (assemblyPath, messages) => {
                ...
                string unetCustomRuntimeDll = FindUNetCustomRuntime();
                if (!File.Exists(unetCustomRuntimeDll)) {
                    Debug.LogError("Could not find PlanetaGameLabo.UNetCustom.Runtime.dll, make sure the file is in your project");
                    return;
                }

                if (assemblyPath == unetCustomRuntimeDll) {
                    Debug.Log("Cannot weave PlanetaGameLabo.UNetCustom runtime");
                    return;
                }
                ...
                if (!buildingForEditor) {
                    ...
                    if (Program.Process(unityEngineCoreModuleDLL, unetCustomRuntimeDll, outputDirectory, new string[] { assemblyPath }, GetExtraAssemblyPaths(assemblyPath), assemblyResolver, Debug.LogWarning, Debug.LogError)) {
        ...
        static string FindUNetCustomRuntime() {
            // we can't assume that PlanetaGameLabo.UNetCustom.Runtime.dll is always at the same
            // path, because some people might move the 'PlanetaGameLabo/UNetCustom' folder into
            // another folder, etc.
            ...
            string key = PlayerSettings.productGUID + ".LastPlanetaGameLaboUNetCustomRuntimeDll";
            ...
            // search directory
            string[] files = Directory.GetFiles("Assets", "PlanetaGameLabo.UNetCustom.Runtime.dll", SearchOption.AllDirectories);
            if (files.Length > 0) {
